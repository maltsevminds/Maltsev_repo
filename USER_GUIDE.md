...
В левой колонке **Strategy** — 7 готовых стратегий.

#### Трендовые / классические
...
#### Скальпинг (M15) — для таймфрейма 15 минут

5. **Bollinger Scalp (M15)** (Mean Reversion)
   - Параметры: BB Period (20), Std Dev (2.0)
   - Вход: цена отскакивает обратно выше нижней полосы Боллинджера
   - Выход: возврат к средней линии (SMA)
   - Логика: ловим перепроданный отскок, фиксируем у средней

6. **Stochastic + EMA Scalp (M15)** (Momentum + фильтр тренда)
   - Параметры: %K (14), %D (3), Trend EMA (50), Oversold (25), Overbought (75)
   - Вход: %K пересекает %D вверх в зоне перепроданности, цена выше EMA(50)
   - Выход: %K пересекает %D вниз в зоне перекупленности
   - Логика: вход только по тренду, тайминг по стохастику

7. **VWAP Bounce Scalp (M15)** (Mean Reversion от VWAP)
   - Параметры: VWAP Window (48 баров ≈ 12ч), Deviation % (0.4%)
   - Вход: цена ушла ниже полосы отклонения от VWAP и вернулась
   - Выход: возврат к самой VWAP
   - Логика: отскок от растянутого отклонения к средневзвешенной цене

> 💡 **Для скальпинга на M15:** в источнике данных выбери таймфрейм **15m**
> (стоит первым в списке). Бери 2000–5000 баров для статистики.
